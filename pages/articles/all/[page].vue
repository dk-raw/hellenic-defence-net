<template>
  <div class="container">
    <h1 class="display-3 fw-bold mb-0">Όλα</h1>
    <hr />
    ...
    <ul class="pagination justify-content-center">
      <li class="page-item" v-if="parseInt(route.params.page) > 1">
        <NuxtLink :to="`/articles/all/${parseInt(route.params.page) - 1}`" class="page-link">&laquo; Προηγούμενη
        </NuxtLink>
      </li>
      <li class="page-item">
        <a class="page-link" href="#">{{ route.params.page }}</a>
      </li>
      <li class="page-item" v-if="posts.length == 30">
        <NuxtLink :to="`/articles/all/${parseInt(route.params.page) + 1}`" class="page-link">Επόμενη &raquo;</NuxtLink>
      </li>
    </ul>
  </div>
</template>
...
const query = `
  {
  articleCollection(skip: ${(page.value - 1) * 30}, limit: 30) {
  total  
  items {
    title
    description
